...
export default class JawBoneContent extends Component {
	...
	componentWillMount() {
		(this.suppressOriginalsJaw = h.get('netflix.ui.akira.originals.jawbone.disable') || !1),
			(this.videoModel = u.assign({}, this.props.videoModel, this.getVideoModel(this.state)));
	}
	...
	getVideoModel(e) {
		...
		if (this.props.hasVideoMerchInJaw && !a.videoMerchId) {
			...
			(a.videoMerchId = l.id),
				(a.videoMerchAspectRatio = '16x9'),
				(a.videoMerchComputeId = l.computeId),
				(a.videoMerchStartTime = l.start);
		}
		return a;
	}
	animateInfo(e, t) {
		if (!this.props.isFirstJaw)
			for (var i = document.getElementsByClassName('has-jawbone-nav-transition'), o = 0; o < i.length; o++)
				(t.target = i[o]), V.animate(t);
	}
	...
	getDefaultPane(e) {
		var t = this.context.routeHandler.query,
			...
			r = void 0;
		return !D && u.get(window, 'location.hash', '').indexOf('showEpisodes') >= 0
			? 'Episodes'
			: A.indexOf(t.jbt) >= 0
			? o || 'Warning' !== t.jbt
				? t.jbt
				: 'Overview'
			: (-1 !== a.indexOf(t.jbp) && ((r = t.jbp), delete t.jbp), r || n);
	}
	getInitialState() {
		return {
			data: void 0,
	...
	getOveriewPane() {
		return N.isRichOriginalsRow(this.context.isTallRow, this.context.models.truths) &&
			!this.props.hasVideoMerchInJaw
			? g
			: m;
	}
	getVisiblePane(e, t) {
	...
	_isClickFromMenu(e) {
		return !!(
			(e.parentElement && 'menu' === e.parentElement.className) ||
			(e.parentElement.parentElement && 'menu' === e.parentElement.parentElement.className)
		);
	}
	prefetchOnLoad() {
		var e = this,
		...
		d &&
			((r = d.startSession('jawbone' + i)),
			(l = function() {
				B.trackBySelectors(['image-rotator-image'])
					.then(function() {
						d.endSession(r);
					})
					.catch(function(e) {
			...
			i && !n[i]
				? (this.mountPrefetchTimeout = setTimeout(function() {
						var t,
							r = o(e.context.models, F.allJawBonePanes, {
						...
						e.setState({
							...
							(t = e.props.model).get.apply(t, (0, s.default)(r)).subscribe(K, d, d),
							(n[i] = !0),
							e.setState({
								hasPaneData: n
							});
	...
	onPlayerError(e) {
		this.props.logStartPlayError &&
			...
			this.handleVideoMerchPlayerComplete();
	}
	onPlayerPlaying() {
		this.state.videoMerchPlaying ||
			this.setState({
				videoMerchPlaying: !0
			});
	}
	...
	logVideoMerchStartPlay() {
		this.context.playerApp.getActionCreators().setStartTime({
			...
			this.props.logStartPlayStarted &&
				this.props.logStartPlayStarted({
	...
	logVideoMerchCancelPlay() {
		if (this.props.logStartPlayCancel) {
			!this.context.playerApp.getAPI().videoPlayer.isVideoPlayingForSessionId(this.props.playerSessionId) &&
	...
	componentDidMount() {
		...
		a && a.on(a.EVENTS.SHOW_EPISODES, this.showEpisodesTab),
			i &&
				O.show(i, function(e) {
					t.animateInfo(e, {
						before: {
	...
	shouldComponentUpdate(e, t, i) {
		return (
			...
			this.state.visiblePane !== t.visiblePane ||
			this.state.isLoading !== t.isLoading ||
			this.props.videoMerchPlayer !== e.videoMerchPlayer ||
			this.state.videoMerchPlaying !== t.videoMerchPlaying ||
			(u.includes(_, this.state.visiblePane) && this.context.columnsInRow !== i.columnsInRow)
		);
	}
	componentDidUpdate() {
		this.prefetchOnLoad();
	...
	componentWillUnmount() {
		...
		t &&
			...
			o && o.removeListener(o.EVENTS.SHOW_EPISODES, this.showEpisodesTab),
			this.props.hasVideoMerchInJaw &&
				(this.logVideoMerchCancelPlay(),
				this.props.closePlayer && this.props.closePlayer(this.props.playerSessionId));
	}
	componentWillReceiveProps(e) {
		var t = {};
		e.videoId !== this.props.videoId &&
			((t.isShow = this.getHasEpisodes()), (t.visiblePane = this.getDefaultPane(e.model))),
			this.props.videoMerchPlayer !== e.videoMerchPlayer && (t.videoMerchPlaying = !1),
			this.setState(t);
	}
	...
	renderTitle(e, t, i) {
		...
		return React.createElement(
			this.props.isStandaloneJawbone ? 'h1' : 'h3',
			{},
			React.createElement(
				I,
				{
	...
	render() {
		...
		return React.createElement(
			...
			React.createElement(b, {
				...
				isStandalone: !h,
				isFullBleed: g,
				videoMerchPlayerElement:
					!!this.videoModel.videoMerchId &&
					(this.props.videoMerchPlayer || this.props.isStandaloneJawbone) &&
