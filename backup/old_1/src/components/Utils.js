...
export function onBinding() {
	for (var n = [], i = 0; i < arguments.length; i++) {
		...
		if (t) {
			...
			else if ('object' === f) for (var o in t) hasOwnProperty.call(t, o) && t[o] && n.push(o);
		}
	}
	return n.join(' ');
}
...
class sharedUtils {
	...
	enableProfileGateSessionTracking() {
		this.profileGateSessionTrackingIsEnabled ||
			(this.throttledResetProfileGateCookieExpiry ||
				(this.throttledResetProfileGateCookieExpiry = f.throttle(
					this.resetProfileGateCookieExpiry.bind(this),
					k
				)),
			this.resetProfileGateCookieExpiry(),
			clearTimeout(this.profileIntervalPlayback),
			window.addEventListener('keydown', this.throttledResetProfileGateCookieExpiry, !1),
			window.addEventListener('mousemove', this.throttledResetProfileGateCookieExpiry, !1),
			(this.profileGateSessionTrackingIsEnabled = !0));
	}
	disableProfileGateSessionTracking() {
		this.profileGateSessionTrackingIsEnabled &&
			...
			clearTimeout(this.profileIntervalPlayback),
			a(),
			this.throttledResetProfileGateCookieExpiry.cancel(),
			window.removeEventListener('keydown', this.throttledResetProfileGateCookieExpiry, !1),
			window.removeEventListener('mousemove', this.throttledResetProfileGateCookieExpiry, !1),
			(this.profileGateSessionTrackingIsEnabled = !1));
	}
	pauseProfileGateSessionTracking() {
		clearTimeout(this.profileTimeout),
			window.removeEventListener('keydown', this.throttledResetProfileGateCookieExpiry, !1),
			window.removeEventListener('mousemove', this.throttledResetProfileGateCookieExpiry, !1),
			(this.profileIntervalPlayback = setInterval(this.resetProfileGateCookieExpiry.bind(this), g));
	}
	updateProfileGateAppState(e) {
		...
		void 0 !== t &&
			(s({
				...
				? this.disableProfileGateSessionTracking()
				: this.enableProfileGateSessionTracking()),
			'function' == typeof this.updateProfileGateAppStateInReact && e && this.updateProfileGateAppStateInReact(e);
	}
	switchProfile(i, a) {
		var n = this,
		...
		if (s && !s.isActive)
			this.updateProfileGateAppState({
				...
				A(t.getActionCreators(), {
					...
					.dispatchStartAction(s)
					.wrapObservable(o(e, i))
					.subscribe(
						function(e) {
			...
			l.inProfileOnrampAATest
				? (window.document.location = d.profilesSetup.makePath())
				: (window.document.location = d.onRamp.makePath());
	...
	onProfileSwitchSuccess(e, t, o) {
		...
		if (
			(i.getActionCreators().clearPinSessions(),
			this.setProfileEditStorageData(),
			r(this.profileIdleTimeout('s')),
			t)
		)
			...
			if (o.showOnRamp)
				return (
					f.get(window, 'netflix.reactContext.models.truths.data', {}).inProfileOnrampAATest
						? (window.document.location = d.profilesSetup.makePath())
						: (window.document.location = d.onRamp.makePath()),
					!0
				);
	...
	startEventListeners(e, i, o) {
		var r = u.get('netflix.ui.akira.enable.crossTab.profile.updation') || !1,
			a = this;
		(this.updateProfileGateAppStateInReact = e),
			(this.models = o),
			P ||
				(r &&
					window.addEventListener('storage', function(e) {
						'profile-edit' !== e.key || a.currentTab
							? 'profile-edit' === e.key && (a.currentTab = !1)
							: window.location.reload();
					}),
				p.on(R.PLAYBACK_START, this.pauseProfileGateSessionTracking.bind(this)),
				p.on(R.PLAYBACK_END, this.enableProfileGateSessionTracking.bind(this)),
				s(i)
					? (this.redirectPageAfterSwitch = window.location.pathname + window.location.search)
					: this.enableProfileGateSessionTracking(),
